...
def migrate_db(db_engine: sqlalchemy.Engine):
    ...
    # do_skip_already_backfilled=True: skip if annotations already exist.
    # Set to False to force re-run -- safe because backfills are idempotent
    # (anti-join: only inserts missing rows, never duplicates).
